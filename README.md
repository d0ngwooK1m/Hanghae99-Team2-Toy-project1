...
> AWS 인스턴스에서 MongoDB가 연결되지 않는 문제가 발생하였습니다. 
> 다시 인스턴스에 git clone을 하고 MongoDB를 다시 연결해서 문제를 해결할 수 있었습니다.

####2. user 패키지 문제
> user 폴더를 패키지화 시켜서 기능을 나누었는데, 이것이 400 error를 유발한 원인으로 추정됩니다.
